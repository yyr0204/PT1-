...
<% 
    ...
    count = dao.getEventCount();	//전체 글의 개수를 리턴하는 메서드
    if (count > 0) {
		...
		<table >
			<tr>
				...
				<td align="left">
				 ...
				 -->
				<td align="right" >
				<%if(admid==null){			//관리자가 아닐때%>	
					<!-- <a href="/pt1/admin/adminloginForm.jsp">로그인</a>-->
					<!-- <a href="/pt1/main/main.jsp">홈페이지</a> -->
					
				<%}else{											//관리자 일때 %>
					<a href="eventWriteForm.jsp">글쓰기</a>
					<!-- <a href="/pt1/main/main.jsp">홈페이지</a> -->
				<%} %>
				</td>
			</tr>
    		...
    		<%for(int i =0 ; i < eventList.size() ; i++){
    			...
    			<tr height="30">
    				<td width="50" > <%=number--%></td>
     				...
     				<td width="150">
     				<a href="eventContent.jsp?num=<%=dto.getNum()%>&pageNum=<%=currentPage%>">
         				  <%=dto.getSubject()%></a> 
     				</td>
    	...
    	<%
    		if (count > 0) {
    			...
    			if (startPage > 10) {%>
    				...
    				<a href="eventMain.jsp?pageNum=<%= startPage - 10 %>">[이전]</a>
    			<%}
    			for (int i = startPage ; i <= endPage ; i++) {  %>
    				<a href="eventMain.jsp?pageNum=<%= i %>">[<%= i %>]</a>
    			<%}
    			if (endPage < pageCount) {  %>
    				<a href="eventMain.jsp?pageNum=<%= startPage + 10 %>">[다음]</a>
    			<%}
    		}
    	%>
	</body>
