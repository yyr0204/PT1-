...
<%}
	...
	if(category == null || category == ""){ // 전체보기나 처음 페이지 들어왔을 때 실행되는 부분
		inquiryList = dao.getSta_inquiry(pageNum);
    	count = dao.s_inquiryCount();
	}
	if(category != null && category != ""){ // 유형 검색하면 실행되는 부분
		inquiryList = dao.getCateinquiry(pageNum, category);
		count = dao.c_s_inquiryCount(category);
	}
...
<body>
	...
	<b>문의유형 검색</b>
	<form action="admin_s_inquiryList.jsp?category=<%=category%>">
	<select name="category">
		<option value="">전체보기</option>
	<%for(int i = 0; i < cate.size(); i++){ 
		InquiryDTO dto = (InquiryDTO)cate.get(i); // 리스트에 있는 값들 dto에 저장
		%>		<%-- 반복문 동안 dto로 저장된 값들 뿌려줌 --%>
		<option value="<%=dto.getCategory()%>"><%=dto.getCategory()%></option>
	<%} %>
	</select>
		<input type="submit" value="검색" />
		<input type="button" value="전체보기" onclick="location='/pt1/inquiry/admininquiryList.jsp'"/>
	</form>
	<table border="1">
		<tr>
			<th>번호</th>
		...
		<tbody>
			<%
	            ...
	            for(int i=0; i < inquiryList.size();i++){
	            	InquiryDTO inquiry = (InquiryDTO)inquiryList.get(i); // 리스트에 있는 값들 dto에 저장
	            	if(inquiry.getRe_step() == 0 && inquiry.getStatus() == 0){ // 답글이 아닌 미처리 상태 글만 출력하기 위한 조건문
	         %>
			<tr>
				<td align="center"><%=number-- %></td>
				<td align="center"><%= inquiry.getWriter() %></td>
				<%-- 글 작성자가 관리자가 아니면 문의유형 출력 --%>
				<%if(!(inquiry.getWriter().equals(admin)) ){ // 작성자가 관리자 세션과 일치하지 않으면 true %>
	...
	<div>
		<%
         ...
         if(totalPage >= 0) {
            if(pageNum > 1) { %>
		<a href="admin_s_inquiryList.jsp?pageNum=<%= pageNum - 1 %>">[이전]</a>
		<% }
            for(int i = 1; i <= totalPage; i++) { %>
		<% if(i == pageNum) { %>
		<a href="admin_s_inquiryList.jsp?pageNum=<%= i %>"><%= i %></a>
		<!-- <strong><%= i %></strong> -->
		<% } else { %>
		<a href="adminin_s_quiryList.jsp?pageNum=<%= i %>"><%= i %></a>
		<% } %>
		<% }
            if(pageNum < totalPage) { %>
		<a href="adminin_s_quiryList.jsp?pageNum=<%= pageNum + 1 %>">[다음]</a>
		<% }
         } %>
